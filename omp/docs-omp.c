...
void atribuirDocs();
void calcularMedia();
void moverDocumento();//Dentro desta fun��o vamos calcular as dist�ncias
...
int main(int argc, char **argv){
	...
	if (argc < 2) {
	    printf("Erro: nome do arquivo de entrada n�o especificado.\n");
	    return 1;
	}
	...
	// verifica se foi passado o n�mero de arm�rios como argumento
	if (argc >= 3) {
...
//Funcao para calcular m�dia, de cada assunto do armarios, com base nos documentos existentes neles
void calcularMedia(){
	
	int k, i, j, l;//i-itera as linhas, j-itera as colunas, k-conta armarios, l-conta as m�dias
	float soma = 0.0;
	
	#pragma omp parallel
	{
	#pragma omp for private(k, i, k, l, soma)
	for(k = 0; k < C; k++){
		l = 0;
...
//Essa funcao � para varrer sempre o vetor dos armarios que guarda as m�dias. como as m�dias variam com base nos documentos, da� a necessidade de varrer sempre que variar.
void inicializarVetor(float * vet){
	int i;
	...
	do{
		...
		for(i = 0; i < D; i++){//Itera os documentos
			...
			for(k = 0; k < C; k++){//itera os armarios
				...
				#pragma omp parallel barrier
				{
					#pragma omp for private(j, q) reduction(+:d)
					...
					//procurar o armario em que o documento a ser movido estava e remov�-lo de l�
					int m, x = 0;
					#pragma omp parallel barrier
					{
						#pragma omp for private(m)
						for(m = 0; m < C; m++){
							if(search_in_list(Cabinets[m].docs, i) > -1 && m != guardaK){//se m <> guardaK ent�o o doc mudou de posi��o ou deve mudar de posi��o no arm�rio
			...
			if(contNaoMudanca == D) 
				...
				calcularMedia(); //Recalcula as m�dias em todos armarios
...
//Funcao de sa�da
//Nota: esta func�o n�o podemos paralelizar pois embora seja bem poss�vel, porque a ordem na imprens�o importa
void gravarSaida(){
	...
	for(i = 0; i < D; i++){
		for(k = 0; k < C; k++){
			if(search_in_list(Cabinets[k].docs, i) > -1){
				arquivo = fopen("docs.out", "a"); // abre o arquivo para adi��o
